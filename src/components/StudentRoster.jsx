...
export default function StudentRoster({ students, setStudents }) {
  ...
  function doImport() {
    ...
    const newStudents = rows
      ...
      .map(r => ({ id: uid(), firstName: r[fi]?.trim() || "", lastName: r[li]?.trim() || "", grade: r[gi]?.trim() || "" }));
  ...
  function addManual(e) {
    ...
    setStudents(s => [...s, { id: uid(), ...manualForm, firstName: manualForm.firstName.trim(), lastName: manualForm.lastName.trim(), grade: manualForm.grade.trim() }]);
    setManualForm({ firstName: "", lastName: "", grade: "" });
  }

  function startEdit(s) { setEditId(s.id); setEditVal({ firstName: s.firstName, lastName: s.lastName, grade: s.grade }); }
  function saveEdit(id) {
    setStudents(s => s.map(x => x.id === id ? { ...x, ...editVal } : x));
    setEditId(null);
  }
  function remove(id) { setStudents(s => s.filter(x => x.id !== id)); }
    ...
    <div>
      <div className="flex items-center justify-between mb2">
        <div>
          ...
          <p className="text-muted" style={{ fontSize: "0.8rem" }}>Import via CSV or manage manually. Roster powers G-Men search &amp; Hall Pass.</p>
        </div>
        <div className="flex items-center gap1">
          <span className="tag tag-gold">{students.length} students</span>
          <button className="btn btn-ghost btn-sm" onClick={() => fileRef.current?.click()}>Import Again</button>
        </div>
      </div>

      <input type="file" accept=".csv,.tsv,.txt" ref={fileRef} style={{ display: "none" }} onChange={e => e.target.files[0] && handleFile(e.target.files[0])} />

      {/* Drop zone (shown when no students yet OR after import button) */}
      {students.length === 0 && !parsed && (
        <div
        ...
        >
          ...
          <div className="text-muted mt1" style={{ fontSize: "0.78rem" }}>Needs at least: First Name, Last Name, Grade columns (any delimiter)</div>
        </div>
      )}
      ...
      {parsed && (
        <div className="card mb2">
          ...
          <p className="text-muted mb1" style={{ fontSize: "0.82rem" }}>Found {parsed.rows.length} rows · {parsed.headers.length} columns. Map your columns:</p>
          ...
          <div style={{ marginBottom: "1rem", overflowX: "auto" }}>
            <table className="stu-table">
              <thead><tr><th>First Name</th><th>Last Name</th><th>Grade</th></tr></thead>
              <tbody>
          ...
          <div className="flex gap1">
            ...
            <button className="btn btn-primary" onClick={doImport}>Import {parsed.rows.length} Students</button>
          </div>
        </div>
      )}
      ...
      {imported && (
        <div className="card mb2" style={{ borderLeft: "4px solid #16a34a" }}>
          <span className="text-green bold">✓ Roster imported successfully — {students.length} students loaded.</span>
        </div>
      )}
      ...
      <div className="card mb2">
        ...
        <form onSubmit={addManual} className="flex gap1 items-center" style={{ flexWrap: "wrap" }}>
          <input style={{ flex: 1, minWidth: 120 }} value={manualForm.firstName} onChange={e => setManualForm(f => ({ ...f, firstName: e.target.value }))} placeholder="First name" />
          <input style={{ flex: 1, minWidth: 120 }} value={manualForm.lastName} onChange={e => setManualForm(f => ({ ...f, lastName: e.target.value }))} placeholder="Last name" />
          <input style={{ flex: "0 0 80px" }} value={manualForm.grade} onChange={e => setManualForm(f => ({ ...f, grade: e.target.value }))} placeholder="Grade" />
      ...
      {students.length > 0 && (
        <div className="card">
          <div className="flex items-center gap1 mb1" style={{ flexWrap: "wrap" }}>
            <input style={{ flex: 1, minWidth: 200 }} value={search} onChange={e => setSearch(e.target.value)} placeholder="Search by name…" />
          ...
          <table className="stu-table">
            <thead><tr><th>#</th><th>First Name</th><th>Last Name</th><th>Grade</th><th>Actions</th></tr></thead>
            <tbody>
              {filtered.length === 0 && <tr><td colSpan={5} className="text-muted" style={{ textAlign: "center", padding: "1rem" }}>No students match your search.</td></tr>}
              {filtered.map((s, i) => (
                <tr key={s.id}>
                  {editId === s.id ? (
                    <>
